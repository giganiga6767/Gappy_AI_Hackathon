  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-56 bg-surface border-r-2 border-ink flex flex-col justify-between">
      <div className="p-6 border-b-2 border-ink">
        ...
        <p className="font-mono text-[10px] text-inkLight mt-1">v1.0.0 // CALM_WORKSPACE</p>
      </div>
      
      <nav className="flex-1 overflow-y-auto py-4">
        <ul className="flex flex-col gap-1 px-4">
          {navItems.map((item) => {
            const isActive = location.startsWith(item.path) || (item.path === "/today" && location === "/");
            return (
              <li key={item.path}>
                <Link 
                  href={item.path} 
      ...
      <div className="p-4 border-t-2 border-ink bg-surface">
        <Link 
          href="/inbox" 
          className="brutal-btn-sage w-full block text-center font-mono text-xs py-2 bg-sage text-paper"
        >
          + QUICK CAPTURE
        </Link>
      </div>
