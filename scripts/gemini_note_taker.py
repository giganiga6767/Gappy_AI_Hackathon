...
if __name__ == "__main__":
    if len(sys.argv) >= 4 and sys.argv[1] == "--md-only":
        md_file = Path(sys.argv[2])
        docx_path = Path(sys.argv[3])
        if not md_file.exists():
            sys.exit(f"❌ Input markdown file not found: {md_file}")
        markdown_content = md_file.read_text(encoding="utf-8")
        save_notes_as_docx(markdown_content, docx_path)
        print(f"✅ Converted markdown to styled DOCX -> {docx_path}")
        sys.exit(0)
