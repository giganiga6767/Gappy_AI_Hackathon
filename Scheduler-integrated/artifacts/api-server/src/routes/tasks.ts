...
router.post("/tasks", async (req, res): Promise<void> => {
  const { title, description, dueDate, category, priority, tags, linkedCourseId, linkedProjectId, confidenceScore, reasoningQuote } = req.body;
  if (!title || !category) {
  ...
  const [row] = await db.insert(tasksTable).values({
    ...
    ...(reasoningQuote !== undefined && { reasoningQuote }),
  }).returning();
...
router.patch("/tasks/:taskId", async (req, res): Promise<void> => {
  const { title, description, dueDate, status, category, priority, tags, confidenceScore, reasoningQuote } = req.body;
