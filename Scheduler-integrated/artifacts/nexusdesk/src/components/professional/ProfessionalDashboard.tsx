  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = () => {
        ...
        const newRecording = {
          id: tempId,
          label: `${recordMode === "zoom" ? "Zoom Session" : "Meeting Audio"} #${recordings.length + 1}`,
      ...
      const res = await fetch("/api/record/start", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ label: recordMode === "zoom" ? "Zoom Session" : "Meeting Recording" }),
      });
      const data = await res.json();
      if (data.success) {
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  ...
                  <a
                    ...
                    className={`p-1.5 border-2 border-ink bg-paper hover:bg-surface active:translate-x-[1px] active:translate-y-[1px] flex items-center justify-center cursor-pointer text-ink ${rec.processing ? "opacity-50 cursor-not-allowed" : ""}`}
                  ...
                  <button
                    ...
                    disabled={rec.processing}
                    className={`p-1.5 border-2 border-ink text-ink font-bold hover:bg-surface active:translate-x-[1px] active:translate-y-[1px] flex items-center gap-1 cursor-pointer disabled:opacity-50 ${rec.showTranscript ? "bg-sageLight" : "bg-paper"}`}
                    title="View Summary"
                  >
